...
const SpecialsPromotionPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await specialService.create(formData);
      fetchPromotions();
  ...
  const confirmDelete = async () => {
    ...
    try {
      await specialService.delete(promotionToDelete);
      fetchPromotions();
          ...
          <div className="relative w-full max-w-xl overflow-hidden rounded-[3rem] border border-slate-100 bg-white shadow-2xl ring-1 ring-slate-100 animate-in fade-in zoom-in-95 duration-500">
            ...
            <form onSubmit={handleSubmit} className="p-8 md:p-10 space-y-8">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="admin-input-field admin-input-field-icon pl-14 h-14 text-lg font-bold"
                    required
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowForm(false)} className="admin-btn-secondary h-14 flex-1">Discard</button>
                <button
                  type="submit"
                  disabled={loading}
                  className="admin-btn-primary h-14 flex-[2] shadow-xl shadow-red-200"
                >
                  {loading ? (

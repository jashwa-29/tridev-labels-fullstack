...
import { Toast, DeleteModal } from '@/components/admin/ui';
...
const QuotesPage = () => {
  ...
  const fetchQuotes = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      // Fetch only service quotes
      const res = await quoteService.getAll(1, 100, 'service');
      setQuotes(res.data || []);
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await quoteService.delete(deleteId);
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="admin-input-field admin-input-field-icon w-full pl-10 pr-4 py-3 h-12 bg-slate-50 border-slate-200 focus:bg-white transition-all shadow-sm"
            />
          </div>

...
const BlogPage = () => {
  ...
  useEffect(() => {
    const token = (typeof window !== "undefined" ? (typeof window !== "undefined" ? localStorage.getItem("token") : null) : null);
    if (!token) {
      router.push("/");
      return;
    }
      ...
      {showForm ? (
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between border-b border-slate-100 pb-10 mb-10">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <div className="text-[10px] font-bold text-red-600 uppercase tracking-[0.2em] mt-1.5 flex items-center gap-2">
            ...
            <div className="flex gap-3">
              <button type="button" onClick={clearForm} className="admin-btn-secondary h-12">Discard Changes</button>
              <button onClick={handleSubmit} disabled={isSubmitting} className="admin-btn-primary h-12 min-w-[200px] shadow-xl shadow-red-200">
                {isSubmitting ? (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                <div className="xl:col-span-8 space-y-6">
                  <CollapsibleSection title="Foundation & Identity" icon={FileText} isExpanded={expandedSections.basic} onToggle={() => toggleSection('basic')} badge={newBlog.cardTitle || "Requires input"}>
                    <div className="grid gap-8">
                      <div className="space-y-2">
                        ...
                        <input type="text" name="cardTitle" value={newBlog.cardTitle} onChange={handleInputChange} placeholder="Brief card overview title..." className="admin-input-field text-xl font-bold py-4" required />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">Full Page Title (Below Image)</label>
                        <input type="text" name="pageTitle" value={newBlog.pageTitle} onChange={handleInputChange} placeholder="Immersive full-page headline..." className="admin-input-field text-xl font-bold py-4" required />
                      </div>
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">Author Identity</label>
                          <input type="text" name="author" value={newBlog.author} onChange={handleInputChange} className="admin-input-field" placeholder="Lead Researcher" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">Content Category</label>
                          <input type="text" name="category" value={newBlog.category} onChange={handleInputChange} className="admin-input-field" placeholder="Technical/Market/Factory" />
                        </div>
                      </div>
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <input 
                            ...
                            value={newBlog.slug} 
                            disabled 
                            className="admin-input-field admin-input-field-icon pl-10 text-xs font-mono text-slate-500 bg-slate-100/80 cursor-not-allowed opacity-75" 
                    ...
                    <div className="space-y-6">
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <input type="text" value={newBlog.highlightBox.title} onChange={(e) => setNewBlog(p => ({ ...p, highlightBox: { ...p.highlightBox, title: e.target.value } }))} className="admin-input-field" placeholder="Core Intelligence" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">Short Introduction</label>
                          <input type="text" value={newBlog.highlightBox.intro} onChange={(e) => setNewBlog(p => ({ ...p, highlightBox: { ...p.highlightBox, intro: e.target.value } }))} className="admin-input-field" placeholder="Brief executive summary..." />
                        </div>
                      </div>
                        ...
                        <div className="grid gap-3">
                          {newBlog.highlightBox.points.map((point, idx) => (
                            <div key={idx} className="flex gap-3">
                              ...
                              }} className="admin-input-field bg-slate-50 border-transparent focus:bg-white" placeholder={`Point ${idx + 1}`} />
                          ...
                          <button type="button" onClick={() => setNewBlog(prev => ({ ...prev, highlightBox: { ...prev.highlightBox, points: [...prev.highlightBox.points, ""] } }))} className="admin-btn-secondary w-full py-3 border-dashed"><Plus size={16} /><span>Add Critical Point</span></button>
                        </div>
                      </div>
                    </div>
                  </CollapsibleSection>
                  ...
                  <CollapsibleSection title="Content Modules (Extra Sections)" icon={LayoutGrid} isExpanded={expandedSections.additional} onToggle={() => toggleSection('additional')}>
                    <div className="space-y-6">
                        ...
                        <div key={index} className="p-8 bg-slate-50/50 rounded-3xl border border-slate-100 relative group">
                          ...
                          <div className="space-y-6">
                            <div className="space-y-2">
                              ...
                              }} className="admin-input-field bg-white" placeholder="Section Title" />
                            </div>
                            <div className="space-y-2">
                      ...
                      <button type="button" onClick={() => setNewBlog(prev => ({ ...prev, sections: [...prev.sections, {heading: "", content: ""}] }))} className="admin-btn-secondary w-full py-4 border-dashed border-2 bg-white"><Plus size={18} /><span>Integrate Content Module</span></button>
                    </div>
                  </CollapsibleSection>
                  ...
                  <CollapsibleSection title="Frequently Asked Questions (FAQs)" icon={HelpCircle} isExpanded={expandedSections.faqs} onToggle={() => toggleSection('faqs')}>
                    <div className="space-y-4">
                      {newBlog.faqs.map((faq, index) => (
                        <div key={index} className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100 flex gap-4 group">
                          <div className="flex-1 space-y-4">
                             ...
                             }} placeholder="Crucial Inquiry..." className="admin-input-field bg-white" />
                             <textarea value={faq.answer} onChange={(e) => {
                               const updated = [...newBlog.faqs];
                               updated[index].answer = e.target.value;
                               setNewBlog(prev => ({ ...prev, faqs: updated }));
                             }} placeholder="Authoritative clarification..." className="admin-input-field bg-white min-h-[100px]" />
                          </div>
                          <button onClick={() => {
                      ...
                      <button type="button" onClick={() => setNewBlog(p => ({ ...p, faqs: [...p.faqs, {question: "", answer: ""}] }))} className="admin-btn-secondary w-full py-4 border-dashed border-2 hover:border-red-600/30 hover:bg-red-50/5"><Plus size={18} /><span>Add FAQ Module</span></button>
                    </div>
                  </CollapsibleSection>
                </div>
                ...
                <div className="xl:col-span-4 space-y-6">
                  ...
                  <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-xl ring-1 ring-slate-100">
                    ...
                    <div className="relative">
                      <input type="text" value={newBlog.newTag} onChange={(e) => setNewBlog(p => ({ ...p, newTag: e.target.value }))} onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addTag())} placeholder="Add taxonomy entry..." className="admin-input-field pr-12" />
                      <button onClick={addTag} className="absolute right-2 top-1/2 -translate-y-1/2 admin-btn-primary !h-8 !px-3 !rounded-lg !text-[10px]">Add</button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <>
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between border-b border-slate-100 pb-10">
            ...
            <button onClick={() => setShowForm(true)} className="admin-btn-primary h-14 px-8 shadow-xl shadow-red-200">
              <Plus size={20} />
          ...
          <div className="space-y-10 mt-12">
              ...
              <div className="hidden md:flex gap-2">
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="admin-input-field admin-input-field-icon !h-11 pl-12 w-64 !bg-white" 
                  />
                </div>
              </div>
            </div>
            ...
            {isLoading ? (
                    ...
                    <div key={blog._id} className="group relative flex flex-col overflow-hidden rounded-[3rem] bg-white shadow-sm ring-1 ring-slate-100 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 ">
                      ...
                      <div className="p-8 flex-1 flex flex-col">
                        ...
                        <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                          <Link href={`/admin/dashboard/blogs/${blog.slug}`} className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-red-600 transition-colors"><Eye size={16} />Preview</Link>
              ...
              <div className="flex h-[500px] flex-col items-center justify-center rounded-[4rem] border-2 border-dashed border-slate-100 bg-white p-12 text-center">
                ...
                <button onClick={() => setShowForm(true)} className="mt-10 admin-btn-primary h-14 px-10">Start First Publication</button>
              </div>
            )}
          </div>
        </>
